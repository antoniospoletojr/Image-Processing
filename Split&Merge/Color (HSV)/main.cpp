...
void Region::calcMean()
{
    ...
    for(int i=rowStart; i<=rowEnd; i++)
    {
        ...
        {
            meanH += image.at<Vec3b>(i,j)[0];
        }
    }
    meanH = meanH/((square.height+1)*(square.width+1));
}
...
void Region::calcVar()
{
    ...
    for(int i=rowStart; i<=rowEnd; i++)
    {
        for(int j=colStart; j<=colEnd; j++)
        {
            varH+=pow((image.at<Vec3b>(i,j)[0]-meanH),2);
        }
    }
    varH = varH/((square.height+1)*(square.width+1));
}
...
void merge()
{
    for(int i=0; i<regions.size(); i++)
    {
        ...
        {
            Region& neighbour = *(*it);
            if(abs(current.meanH-neighbour.meanH)<10)
                neighbour.meanH = (neighbour.meanH+current.meanH)/2;
        }
    }
}
...
void print()
{
    ...
    for(int k=0; k<regions.size(); k++)
    {
        for(int i=regions.at(k).square.y; i<regions.at(k).square.y+regions.at(k).square.height; i++)
            ...
            {
                outputHSV.at<Vec3b>(i,j)[0] = saturate_cast<uchar>(regions.at(k).meanH);
                outputHSV.at<Vec3b>(i,j)[1] = image.at<Vec3b>(i,j)[1];
                outputHSV.at<Vec3b>(i,j)[2] = image.at<Vec3b>(i,j)[2];
            }
    }
    outputBGR = outputHSV.clone();
    cvtColor(outputHSV,outputBGR, COLOR_HSV2BGR);
