...
void Region::mapAdjacents()
{
	for(int k=0; k<regions.size(); k++)
    {
        ...
        //calcola i 4 punti medi dei lati della k-sima regione
        //(questi punti sono calcolati in modo da cadere fuori dalla regione i-sima)
        for(int side=0; side<4; side++)
        {
            midPoints.push_back(getMidPoint(current,side));
        }
        for(int l=0; l<regions.size(); l++) //per ogni altra regione l
        {
            Region& neighbour = regions.at(l);
            if(k!=l && (current.square.height<=neighbour.square.height))//se non ti stai confrontando con te stesso e se sono più piccolo di te (ottimizzazione)
            {
                for(int side=0; side<4; side++)
                {	//se la regione l-ima contiente al suo interno un punto medio, allora k e l sono confinanti
...
void split(int rowStart, int rowEnd, int colStart, int colEnd)
{
    ...
    if((rowEnd-rowStart)*(colEnd-colStart) <= AREA_THRESHOLD)
    {
...
void splitAndMerge()
{
    ...
    Region::mapAdjacents(); //calcola tutte le adiacenze
    merge();
    print();
